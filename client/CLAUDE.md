...
- **`dbClient`** — axios targeting `VITE_SUPABASE_URL/rest/v1` (PostgREST). Has a request interceptor that injects the live Supabase JWT as `Authorization`. Uses a custom `paramsSerializer` that produces repeated keys for PostgREST multi-value filters (e.g. `log_date=gte.X&log_date=lte.Y`).
- **`offClient`** — axios targeting Open Food Facts.
...
Three tables (canonical source: `supabase/reference/curr_db_schema.sql`):
- `profiles` — `id` (FK → auth.users), `name`, `calorie_goal`, `protein_goal`, `carbs_goal`, `fat_goal`
...
- Three colors use CSS variables (`--c-primary`, `--c-outline-variant`, `--c-surface-dim`) so they support Tailwind opacity modifiers (`bg-primary/30`) and swap in dark mode. All other colors are fixed hex in `tailwind.config.js`.
...
- Typography: `font-display` (Plus Jakarta Sans) for headlines; `font-sans` (Inter) for body.
